...
local game    = require "pdkroom.game"
...
local function push_offline_cache(player)
  ...
  elseif rfsm:is('room_waiting') then
    ...
    player.send_msg("pushCache", {pdk = {state = "READY",  readyDatas = readyDatas}})
...
function rfsm:onready()
  ...
  game.start(players, ROOM_ID, ROOM_TYPE, MAX_USER, showType, mustType)
end
...
function command.ENTER(source, agent, fd, profile, uin, secret, retry)
  ...
  for _,v in ipairs(players) do
    if v.ready then
        player.send_msg("pushRoundPlayerState", {pdk = {
            uin = v.uin,
            state = "PDKREADY",
        }})
    end
...
function command.PLAYER_READY(source)
  ...
  player.ready = true
  response_ok(player, "playerReady")
  notify_players("pushRoundPlayerState", {pdk = { uin = player.uin, state = "PDKREADY" }}, player.uin)
