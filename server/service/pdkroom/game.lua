...
local function dealRoundResult()
  ...
  for _,v in ipairs(players) do
     ...
     table.insert(result, { uin = v.uin, score = v.scores[#v.scores], cards_count = #v.cards, bomb = v.bomb, name = v.profile.name })
...
local function gen_room_record(roomResult)
  ...
  if #G.round_result == 0 then 
    local result = {}
    for _,v in ipairs(players) do  
     table.insert(result, { uin = v.uin, score = 0, cards_count = #v.cards, bomb = v.bomb, name = v.profile.name })
    end
    G.round_result[round_id] = {}
    G.round_result[round_id].result = result
    G.round_result[round_id].ts = G.round_start_time[round_id]
  ...
  -- ts 等于房间开始时间
  for _, v in ipairs(players) do
    mongox.safe_insert("player_room_record", { uin = v.uin, room_db_id = ret._id, ts = G.room_start_time, game_type = "PDK_GAME" })
  end
end
