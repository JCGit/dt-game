...
local function dealPlayerResult()
  ...
  if G.winMode =="SPRING" then
    for _,v in ipairs(players) do
      if v.uin ~= G.dealer.uin then 
        score = (G.WINNER.uin == G.dealer.uin) and - G.dealer.rob or 
                        G.dealer.rob 
        v.score = v.score +score
        table.insert(v.scores, score)
      end
    end 
    score =  (G.WINNER.uin == G.dealer.uin) and  
  ...
  for _,v in ipairs(players) do
    table.insert(result, { uin = v.uin, score = v.scores[#v.scores], bomb = v.bomb, name = v.profile.name})
...
function gfsm:onround_running()
  if not G.isVoteResume then
    ...
    G.prev_cue = {}
    G.prev_cue_uin = nil
    notify_players("pushPlayerOperate", {teg = {uin = G.dealer.uin, type = "TEG_PLAY", first = true }})
  end
end
...
local function gen_room_record(roomResult)
  if #G.round_result == 0 then 
    local result = {}
    for _,v in ipairs(players) do  
     table.insert(result, { uin = v.uin, score = 0, bomb = v.bomb, name = v.profile.name })
    end
    G.round_result[G.round_id] = {}
    G.round_result[G.round_id].result = result
    G.round_result[G.round_id].ts = G.round_start_time[G.round_id]
  ...
  -- ts 等于房间开始时间
  for _, v in ipairs(players) do
    mongox.safe_insert("player_room_record", { uin = v.uin, room_db_id = ret._id, ts = G.room_start_time, game_type = "TEG_GAME"})
  end
end
...
local function play_card(player, cards)
  local function play(player, cards)
    ...
    if card_type == card_constant.CARD_TYPES.Bomb then
    ...
    local temp_cards = {}
    if card_type == "THREE" or card_type == "PLANE" then
      for _,v in ipairs(groups) do 
        for k,c in ipairs (v) do 
          table.insert(temp_cards, c[3])
        end
      end
    elseif card_type == "FOUR" then
      for _,v in ipairs(groups) do  
        table.insert(temp_cards, v[3])
      end
    end
    
    return "ok", card_type, temp_cards
  end

  local ret, card_type, temp_cards = play(player, cards)
  
  if temp_cards and #temp_cards > 0 then
    cards = utils.simple_copy_obj(temp_cards)
  end
  ...
  if G.winMode == "SPRING" then
    if #cards > 0 and player.uin ~= G.dealer.uin then
      G.WINNER = player
      for _,v in ipairs(players) do
  ...
  if remain > 0 then
   nextPlay()
    ...
    G.WINNER = player
    if G.winMode == "SPRING" and G.WINNER.uin == G.dealer.uin then
      notify_players("pushRoundPlayerState", { teg = { uin = G.WINNER.uin, state = "TEG_SPRING"}})
    end
    for _,v in ipairs(players) do
...
function game.push_cache(player)
  ...
  local state, current_play_uin, ready, first, robData, canOper
