...
function M.searchCard(cards, prevCardType, prevCard)
  ...
  if prevCardType == "SINGLE" then
    for _,c in ipairs(cards) do 
      if c > prevCard[1] then
        table.insert(CardSet, {card_set = {c}})
      end
    end
  else
    local card_set = getCardSet(cards)
    ...
    for _,g in ipairs(group) do
      if #g >= #prevCardSet then
        ...
        if gs[1][3] > prevCardSet[1][3] and gs[1][2] ~= prevCardSet[1][2] then
          local t = {}
          table.move(g, #g - #prevCardSet + 1, #g, 1, t)
          table.insert(CardSet, {card_set = t})
        end 
      end
    end
  end
  ...
  if #CardSet > 0 then
    table.sort( CardSet, function ( a,b )
      if a.card_set[1] < b.card_set[1] then
        return true
      end
    end )
