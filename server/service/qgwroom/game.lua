...
local function dealRoundResult()
  local result ={}
  for _,v in ipairs (players) do 
    table.insert(result, { uin = v.uin, score = v.score, name = v.profile.name})
...
function gfsm:onround_stoped(event, from, to)
  ...
  --推送结果
  notify_players("pushRoundStoped", {qgw = {playerresult = result}})
...
local function gen_room_record(roomResult)
  if #G.round_result == 0 then 
    local result = {}
    for _,v in ipairs(players) do  
     table.insert(result, { uin = v.uin, score = 0, name = v.profile.name })
    end
    G.round_result[round_id] = {}
    G.round_result[round_id].result = result
    G.round_result[round_id].ts = G.round_start_time[round_id]
  ...
  -- ts 等于房间开始时间
  for _, v in ipairs(players) do
    mongox.safe_insert("player_room_record", { uin = v.uin, room_db_id = ret._id, ts = G.room_start_time, game_type = "QGW_GAME" })
  end
end
...
local function game_stoped()
  ...
  for _, v in ipairs(players) do
     ...
     table.insert(roomResult, {uin = v.uin,  score = v.total_score , name = v.profile.name })
  end
...
local function nextPlay()
  ...
  if G.prev_cards.seatID ~= G.current_play then
    ...
    if G.score > 0 then 
      ...
      players[G.current_play].score = players[G.current_play].score + G.score
      G.score = 0 
      notify_players("pushScore", { isDesktop = true, Score = G.score })  --桌面积分清零
    end

    --补牌
    local function deal_pushCards()
      if #G.cards > 0 then
        local need_cards = {}
        local total_num = 0
        local current_index = G.current_play

        for i = 1, #players do 
          local num = G.every_card_mount - #players[current_index].cards
          need_cards[players[current_index].uin] = num
          total_num = total_num + num
          current_index = next_index(current_index)
        end
        
        if #G.cards < total_num then
          local can_push = {}
          local cardCount = #G.cards
          for _,v in ipairs(players) do 
             can_push[v.uin] = 0
          end
          for i =1,cardCount do
            ...
            current_index = G.current_play
            for i = 1, #players do
              if need_cards[players[current_index].uin] > 0 then
                need_cards[players[current_index].uin] = need_cards[players[current_index].uin] - 1
                can_push[players[current_index].uin] = can_push[players[current_index].uin] + 1
          ...
          need_cards = utils.simple_copy_obj(can_push)

        end
        current_index = G.current_play
        for i = 1, #players do
          local cards = {}
          if need_cards[players[current_index].uin] and need_cards[players[current_index].uin] > 0 then 
            table.move(G.cards, 1, need_cards[players[current_index].uin], 1, cards)
            lib.remove_sublist( G.cards, cards)
            players[current_index].send_msg("pushCards", { uin = players[current_index].uin, cards = cards, cardsCount = #cards })
            notify_players("pushCards", { uin = players[current_index].uin, cardsCount = #cards }, players[current_index])
            table.move(cards, 1, #cards, #players[current_index].cards + 1, players[current_index].cards)
          end
          current_index = next_index(current_index)
        end
...
local function play_card(player, cards)
  local function play(player, cards)
    ...
    if G.prev_cue and #G.prev_cue > 0 and G.prev_cue_uin == player.uin then
      for _,c in ipairs (G.prev_cue) do
        ...
        if index then
          ret = "ok"
          card_type = G.prev_cards.card_type
          break
        end
      end
    end
  ...
  if mScore > 0 then
    ...
    notify_players("pushScore", { isDesktop = true, Score = G.score })  --发总积分 
  ...
  elseif #G.cards == 0 and remain == 0 then 
    ...
    for _,v in ipairs(players) do
      if #v.cards > 0 then
        score = score + lib.getScore(v.cards)  
      end
    end
    
    
    notify_players("pushScore", { isDesktop = false, Score = score, uin = player.uin })
