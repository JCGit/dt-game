...
local game    = require "qgwroom.game"
...
local function push_offline_cache(player)
  ...
  elseif rfsm:is('room_waiting') then
    ...
    player.send_msg("pushCache", {qgw = {state = "READY",  readyDatas = readyDatas}})
...
function rfsm:onready()
  ...
  game.start(players, ROOM_ID, ROOM_TYPE, #players)
end
...
local function agentStartGame()
  ...
  if readyindex > 1  and #players == readyindex then
    rfsm:ready()
  end
end
...
function command.ENTER(source, agent, fd, profile, uin, secret, retry)
  ...
  local function response_client_ok(player)
    --local special = json.encode({showType = showType, mustType = mustType})
  ...
  for _,v in ipairs(players) do
    if v.ready then
        player.send_msg("pushRoundPlayerState", {qgw = {
            uin = v.uin,
            state = "QGW_READY",
        }})
    end
...
function command.VOTE(source, uin, agree, inner)
  ...
  if ( #players == 2 and not agree ) or ( #players >2 and #votes - count >= math.ceil(#players/2) ) then
    notify_players("pushVoteEnd", {status = 2, votes = votes})
    rfsm:resume()
    game.resume()
  elseif ( #players == 2 and #votes == 2 ) or ( #players >2 and count >= math.ceil(#players/2) ) then
...
function command.PLAYER_READY(source)
  ...
  player.ready = true
  response_ok(player, "playerReady")
  notify_players("pushRoundPlayerState", {qgw = { uin = player.uin, state = "QGW_READY" }}, player.uin)
