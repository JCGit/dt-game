...
local function dealPublic(theplayer)
    ...
    if G.isPublic then
       ...
       print("dealPublic", inspect(G.round_result[G.round_id]))
       for _,v in ipairs(G.round_result[G.round_id].result) do
...
function gfsm:onround_stoped(event, from, to)
  ...
  if DEALER_TYPE == "HAVE_DEALER" then
      ...
      for _,v in ipairs(players) do
         if not v.profile.owner then
            ...
            table.insert(resultset, {result = v_result, uin = v.uin, name = v.profile.name, modifyscore = score, cards = v.cards,
      ...
      table.insert(resultset, { result = result, uin = dealer.uin, name = dealer.profile.name, modifyscore = d_score, cards = dealer.cards, 
      ...
      G.round_result[G.round_id].result = resultset
    ...
    for _,v in ipairs(players) do
       if v.uin ~= winner.uin then 
          score = v.result[G.round_id].type * v.power
          table.insert(resultset, { result = "LOSE", uin = v.uin, name = v.profile.name, modifyscore = def_score - score  , cards = v.cards, 
               publicscore = def_score, niutype = NIUTYPE[v.result[G.round_id].type], sum = v.result[G.round_id].sum } )

          w_score = w_score + score
       end
    end
    table.insert( resultset, { result = "WIN", uin = winner.uin, name = winner.profile.name, modifyscore = def_score + w_score, cards = winner.cards, 
  ...
  --推送结果
  notify_players("pushRoundStoped", {bull = {playerresult = resultset}})
  for _,player in ipairs(players) do
...
local function sumResult()
  ...
  for i=1,G.round_id do
    for _,v in ipairs(G.round_result[i].result) do
...
local function gen_room_record()
  ...
  for _,v in ipairs(G.round_result) do
    for _,c in ipairs(v.result) do
      c.score = c.modifyscore + c.publicscore
    end
  end
  local ret = mongox.safe_insert("room_record", {room_id = ROOM_ID, game_type = "DN_GAME", ts = G.ROOM_START_TIME,
    room_result = room_result, round_result = G.round_result})

  -- ts 等于房间开始时间
  for _, v in ipairs(players) do
    mongox.safe_insert("player_room_record", {uin = v.uin, room_db_id = ret._id, ts = G.ROOM_START_TIME, game_type = "DN_GAME"})
  end
end

local function game_stoped()

  if (not G.round_result[G.round_id] and G.round_id == 1) then  --一局都没打完的情况

    for _,v in ipairs(players) do
      table.insert(accounts, {uin = v.uin, modifyscore = 0, niucount = {0, 0, 0, 0} })
      G.round_result[G.round_id] = {}
      local result = {}
      table.insert(result, {uin = v.uin, modifyscore = 0, niutype = nil, sum = 0, name = v.profile.name, result = "TIE", cards = {}, publicscore = 0})
      G.round_result[G.round_id].result = result
      G.round_result[G.round_id].ts = G.round_start_time[round_id]
    end
  end  
...
local function reConnectDeal(player)
    ...
    elseif gfsm:is("round_stoped") then
      if not player.ready then    -- 未准备阶段，推结算
        ...
        player.send_msg( "pushCache", {bull = {state = ROOM_STATE, result = G.round_result[G.round_id].result, roundid = G.round_id }} )   --推送单局结算
      else
        player.send_msg( "pushCache", { bull = {state = ROOM_STATE, 
                            result = G.round_result[G.round_id].result,
                            playercache = playercache,
                            roundid = G.round_id,
                             }} ) 
      end
...
local function publicBetOver()
  ...
  if public_player.power > 0 then
    ...
    temp[index].send_msg("pushPlayCards", {bull = {
                     isPublic = true,
                     cards = public_player.cards,
                     niutype = NIUTYPE[tonumber(public_player.result[G.round_id].type)],
                     sum = public_player.result[G.round_id].sum,}})
